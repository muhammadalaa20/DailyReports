...
export default function Home() {
  ...
  const fetchTasks = async (day, shift) => {
    try {
      const response = await fetch(
        `http://172.17.40.200:8070/dct/reports/${day}/${shift}`
      );
  ...
  const confirmDelete = async () => {
    try {
      await fetch(`http://172.17.40.200:8070/dct/reports/${deleteTarget}`, {
        method: "DELETE",
      });
  ...
  const confirmUpdate = async () => {
    try {
      const response = await fetch(`http://172.17.40.200:8070/dct/tasks/${selectedTask}`, {
        method: "PUT",
        headers: {
  ...
  const deleteTask = async () => {
    ...
    try {
      const response = await fetch(`http://172.17.40.200:8070/dct/tasks/${deleteTaskTarget}`, {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const confirmAddTask = async () => {
    ...
    console.log(task)
    const response = await fetch("http://172.17.40.200:8070/dct/tasks", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
  ...
  const handleLogout = () => {
    logout(); // Clear session via context
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      <nav className="bg-slate-900 text-white p-4  flex justify-between items-center">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-2xl font-bold">DCT Reports</h1>
        </div>
